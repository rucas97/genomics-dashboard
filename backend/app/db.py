"""
Wrapper around Supabase calls with retry on transient network failures.
"""
import time
import httpx
...
def with_retry(fn, *args, retries: int = 3, delay: float = 0.3, **kwargs):
    """
    Call fn(*args, **kwargs) and retry on transient errors.
    The callable should be a lambda that performs the full Supabase request.
    """
    last_exc = None
