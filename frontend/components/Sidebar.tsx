...
import { LayoutDashboard, FlaskConical, Dna, Users, PlayCircle, FileText, Shield, LogOut } from "lucide-react";
        ...
        {nav.map(({ href, label, icon: Icon }) => (
          <Link key={href} href={href}
            className={`flex items-center gap-3 px-3 py-2 rounded-md text-sm transition ${
              path === href ? "bg-slate-800 text-emerald-400" : "hover:bg-slate-900 text-slate-300"
            }`}>
            <Icon size={16} /> {label}
          </Link>
        ))}
      </nav>
      <button onClick={logout} className="flex items-center gap-3 px-3 py-2 rounded-md text-sm text-slate-400 hover:bg-slate-900">
