...
export default function RunDetailClient() {
  ...
  async function deleteRun() {
    if (!run) return;
    if (!confirm(`Delete pipeline run "${run.pipeline_name}"?`)) return;
    setDeleting(true);
    try {
            ...
            <div className="flex items-center justify-between mt-3 mb-6">
              ...
              <div className="flex items-center gap-3">
                ...
                <button
                  onClick={deleteRun}
                  disabled={deleting}
                  className="bg-red-950 hover:bg-red-900 border border-red-900 disabled:opacity-50 px-3 py-1 rounded text-xs font-medium text-red-300"
                >
